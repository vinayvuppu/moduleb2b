...
import EmployeesListCustomViewsConnector from '../../../employees/components/employees-list-custom-views-connector';
import EmployeesListConnector from '../../../employees/components/employees-list-connector';
...
describe('render', () => {
  ...
  describe('when loading', () => {
    beforeEach(() => {
      ...
      wrapper = shallow(<CompanyEmployeesList {...props} />)
      ...
      describe('with `visibleColumns` on `activeView`', () => {
        beforeEach(() => {
          ...
          const activeView = createActiveView({
            visibleColumns: ['customerNumber', 'firstName', 'foo'],
          });
          wrapper = shallow(<CompanyEmployeesList {...props} />)
            .find(B2BApolloClientContext.Consumer)
            .renderProp('children')(createB2BApolloClientContextProps())
...
describe('interacting', () => {
  ...
  describe('`handleRowClick`', () => {
    ...
    it('should call the router with correct parameters', () => {
      expect(props.history.push).toHaveBeenCalledWith(
        '/test/b2b-extension/my-company/employees/1/general'
      );
    });
  });
  ...
  describe('`generateSearchQuery`', () => {
    ...
    beforeEach(() => {
      ...
      searchQuery = wrapper
        .instance()
        .generateSearchQuery(createActiveView(), 'companyKey1');
    });
    it('should add the `company` filter to the search', () => {
      expect(searchQuery).toEqual({
        filters: {
          'customerGroup.id': [{ type: 'equalTo', value: ['companyKey1'] }],
        },
        page: 1,
        perPage: 20,
        searchText: '',
...
describe('callbacks', () => {
  ...
  describe('<ColumnManager>', () => {
    describe('`onUpdateColumns`', () => {
      ...
      it('should invoke `setActiveView` on the `<CompanyEmployeesListCustomViewsConnector>`', () => {
        expect(customViewConnectorProps.setActiveView).toHaveBeenCalled();
      });

      it('should invoke `setActiveView` on `<CompanyEmployeesListCustomViewsConnector>` with keys of the next visible columns', () => {
