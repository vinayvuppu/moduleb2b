...
import createStateReferenceSingleFilter, {
  mapItemToOption,
  renderItem,
  filterOption,
} from './state-reference-filter';
...
jest.mock('../with-dereferenced-state', () => (...args) =>
  mockWithDereferencedResource(...args)
);

describe('named exports', () => {
  describe('mapItemToOption', () => {
    it('should map state fields for dropdown option', () => {
      expect(mapItemToOption({ id: '1', name: 'State 1', key: 'foo' })).toEqual(
        {
          value: '1',
          label: 'State 1',
          key: 'foo',
    ...
    describe('when state has no name', () => {
    ...
    describe('when state has no key', () => {
      it('should return fallback', () => {
  ...
  describe('renderItem', () => {
    let wrapper;
    beforeEach(() => {
      wrapper = shallow(renderItem({ name: 'State 1', key: 'foo' }));
    });
  ...
  describe('filterOption', () => {
    it('should return true if filter value matches', () => {
      expect(filterOption({ label: 'State 1', key: 'foo' }, 'tate')).toBe(true);
    });
    describe('if key is not defined', () => {
      it('should only match by label', () => {
        expect(filterOption({ label: 'State 1' }, 'foo')).toBe(false);
      });
    });
  });
});
...
describe('withDereferencedResource', () => {
  ...
  beforeEach(() => {
    ...
    WrappedComponent = createStateReferenceSingleFilter({});
    wrapper = shallow(
